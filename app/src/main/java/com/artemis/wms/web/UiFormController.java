...
public class UiFormController {
    ...
    public String releaseWave(@RequestParam Map<String, String> f, RedirectAttributes flash) {
        ...
        try {
            ...
            int n = ((List<?>) result.get("assignments")).size();
            return back(flash, n > 0
                ? "Released: " + n + " assignment(s), " + result.get("putMode") + " mode."
                : "Wave released — orders pooled. Combine them into assignments once equipment is known.", to);
        } catch (Exception e) { return fail(flash, e, to); }
    }
